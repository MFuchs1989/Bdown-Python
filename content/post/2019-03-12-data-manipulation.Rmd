...
Just as often it can happen that the structure of the data (e.g. when joining datasets) does not fit.
...
df2 = df.groupby(['Year', 'Season']).sum()
df2
```

![](/post/2019-03-12-data-manipulation_files/p3z12.png)

In such a case the stack function can also be used:

```{r, eval=F, echo=T}
df_stacked = df2.stack().reset_index()
df_stacked.columns = ['Year', 'Season', 
                      'Weather_Information', 
                      'Value']
df_stacked
```

![](/post/2019-03-12-data-manipulation_files/p3z13.png)
...
# 6 Function for colouring negative values
